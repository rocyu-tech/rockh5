...
func main() {
        ...
        api.Get("/health", func(c *fiber.Ctx) error {
                return c.JSON(bizerr.SuccessResponse(map[string]string{"status": "ok", "service": "event-node"}))
        })

        api.Post("/track", placeholder("trackEvent"))
        api.Post("/batch", placeholder("batchEvents"))
        api.Get("/stats", placeholder("getStats"))
